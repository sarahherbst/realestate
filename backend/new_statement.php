<?php
	...
	if (isset($_POST['submit'])) {
		...
		$statement_update 	= sql_insert('statements', array('stm_title', 'stm_text', 'chg_user', 'new_user'), array($stm_title, $stm_text, $user_email, $user_email));
		if ($statement_update == true) {
			$alert .= "<div class='alert alert-success alert-dismissible' role='alert'><button class='close' type='button' data-dismiss='alert' aria-label='Schließen'><span aria-hidden='true'>×</span></button>Das Kundenprojekt wurde erfolgreich hinzugefügt.</div>";
			$stm_title 				= '';
			$stm_text 				= '';

		} else {
			$fehler++;
			$alert .= "<div class='alert alert-danger alert-dismissible' role='alert'><button class='close' type='button' data-dismiss='alert' aria-label='Schließen'><span aria-hidden='true'>×</span></button>Fehler! Das Kundenprojekt konnte leider nicht hinzugefügt werden.</div>";
		}
			...
			if ($fehler == 0) {
				...
				if ($fehler == 0) {
					...
					if ($thumbnail == '' || $thumbnail == false) {
						...
						// Datenbankeintrag
						$statement_sql 	= sql_select_where('all', 'statements', 'stm_text', $_POST['stm_text'], '', '');
						$statement_row 	= mysqli_fetch_assoc($statement_sql);
						$stm_id 		= $statement_row['stm_id'];
...
<ol class="breadcrumb">
	...
	<li class="breadcrumb-item active">Kundenprojekt hinzufügen</li>
</ol>

<div class="jumbotron">
	<h1>Kundenprojekt hinzufügen</h1>
	<p class="lead">Hier können Sie ein weiteres Kundenprojekt hinzufügen.</p>
</div><!-- /.jumbotron -->
				...
				<div class="card-body">
					<div class="row">
						<div class="col-md-6">
							...
							<div class="form-group">
								<label for="stm_text">Text:</label>
								<textarea type="text" class="form-control" name="stm_text" id="stm_text" placeholder="Beschreibung" rows="4" required><?php echo $stm_text; ?></textarea>
							</div>
						</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<p class="help-block">Laden Sie ein Bild des Kundenprojektes hoch.</p>
