<?php
	...
	// Tabelle auslesen
	$immolist_sql = sql_select_where('all', 'texte', array('txt_institut', 'txt_rubrik', 'txt_schluessel'), array($institut_id, $rub_id, $schluessel), '', '');

	if ( mysqli_num_rows($immolist_sql) == 1 ) {
		$immolist_row 				= mysqli_fetch_assoc($immolist_sql);

		// Variablen vergeben
		$txt_titel 				= $immolist_row['txt_titel'];
		$txt_einleitung 		= $immolist_row['txt_einleitung'];
		$txt_einleitung 		= str_replace('<br />', '', $txt_einleitung);
		$txt_einleitung 		= str_replace('\r\n', '', $txt_einleitung);
		$txt_einleitung 		= stripslashes($txt_einleitung);
		$txt_auszug 			= $immolist_row['txt_auszug'];
		$txt_auszug 			= str_replace('<br />', '', $txt_auszug);
		$txt_auszug 			= str_replace('\r\n', '', $txt_auszug);
		$txt_auszug 			= stripslashes($txt_auszug);
		$txt_beitrag 			= $immolist_row['txt_beitrag'];
		$txt_beitrag 			= str_replace('<br />', '', $txt_beitrag);
		$txt_beitrag 			= str_replace('\r\n', '', $txt_beitrag);
		$txt_beitrag 			= stripslashes($txt_beitrag);
		$txt_conversion_ziel 	= $immolist_row['txt_conversion_ziel'];
		$txt_conversion_titel 	= $immolist_row['txt_conversion_titel'];
	} else {
		$txt_titel 				= '';
		$txt_einleitung 		= '';
		$txt_auszug 			= '';
	...
	if (isset($_POST['submit'])) {
		...
		if ($immolist_update == true) {
			...
			$fehler++;
			$alert   		.= '<div class="alert alert-danger alert-dismissible fade in" role="alert"><b>Fehler!</b> Die Immobilienliste für die Rubrik <b>'.$row_rubrik['rub_name'].'</b> konnte nicht gespeichert werden.</div>';
			$txt_einleitung 		= $_POST['txt_einleitung'];
			$txt_auszug 			= $_POST['txt_auszug'];
			$txt_beitrag 			= $_POST['txt_beitrag'];
			$txt_conversion_ziel 	= $_POST['txt_conversion_ziel'];
...
<ol class="breadcrumb">
	...
	<li class="breadcrumb-item active">Immobilienliste bearbeiten</li>
</ol>

<div class="jumbotron">
	<h1>Immobilienliste bearbeiten</h1>
	<p class="lead">Hier haben Sie die Möglichkeit eine Immobilienliste für die Rubrik <b><?php echo $row_rubrik['rub_name']; ?></b> zu zufügen/aktualisieren.</p>
</div><!-- /.jumbotron -->
		...
		<div class="col-md-12">
			<div class="card mb-3">
				<div class="card-header">
					<i class="fa fa-list" aria-hidden="true"></i> &nbsp; Immobilienliste für die Rubrik <b><?php echo $row_rubrik['rub_name']; ?></b>
				</div>
				<div class="card-body">
					<div class="form-group">
						<label for="txt_auszug">IFrame-URL:</label>
						<p class="help-block">Geben Sie die URL zur Immobilienliste an.</p>
						<input type="text" class="form-control" placeholder="URL" name="txt_auszug" value="<?php echo $txt_auszug; ?>" required>
					</div>
					<div class="form-group">
