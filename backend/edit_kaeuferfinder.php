	...
	if (isset($_POST['submit'])) {
		...
		if ($fehler == 0) {
			if ( mysqli_num_rows($txt_sql) == 1 ) {
				...
				$txt_update 	= sql_insert('texte', array('txt_institut', 'txt_schluessel', 'txt_alias', 'txt_titel', 'txt_einleitung', 'txt_rubrik', 'new_user', 'chg_user'), array($institut_id, $schluessel, 'gameplay', $txt_titel, $txt_einleitung, $txt_rubrik, $user_email, $user_email));
			}
		}

		if ($txt_update == true) {
			$fehlerangabe .= '<div class="alert alert-success" role="alert">Die Angaben zum Käuferfinder wurden erfolgreich gespeichert.</div>';
			$txt_einleitung 		= str_replace('<br />', '', $txt_einleitung);
			$txt_einleitung 		= $_POST['txt_einleitung'];
		} else {
			$fehler++;
			$fehlerangabe   .= '<div class="alert alert-danger alert-dismissible fade in" role="alert"><b>Fehler!</b> Die Angaben zum Käuferfinder konnten nicht gespeichert werden.</div>';
			$txt_einleitung 		= str_replace('<br />', '', $txt_einleitung);
			$txt_einleitung 		= $_POST['txt_einleitung'];
		}
	}
...
<!-- Breadcrumbs -->
<ol class="breadcrumb">
	<li class="breadcrumb-item active">Käuferfinder</li>
	<li class="breadcrumb-item active">Angaben zum Käuferfinder bearbeiten</li>
</ol>

<div class="jumbotron">
	<h1>Angaben zum Käuferfinder bearbeiten</h1>
	<p class="lead">Hier haben Sie die Möglichkeit die Angaben zum Käuferfinder zu bearbeiten.</p>
</div><!-- /.jumbotron -->
					...
					<div class="row">
						...
						<div class="col-md-6">
							...
							<div class="form-group">
								...
								<p class="help-block">Geben Sie eine Headline zum Käuferfinder an.</p>
