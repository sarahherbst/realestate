<?php
	...
	if (isset($_POST['submit'])) {
		...
		if ($_FILES['motiv']['error'] <= 0) {
				...
				if ($fehler == 0) {
					...
					if ($thumbnail == '' || $thumbnail == false) {
						...
						if ( mysqli_num_rows($img_sql) == 1 ) {
							$image_upload 	= sql_update('images', array('img_bild', 'img_thumb', 'img_titel', 'img_url', 'img_beschreibung', 'chg_user'), array($large, $thumbnail, $img_titel, $img_url, $img_beschreibung, $user_email), array('img_institut', 'img_id', 'img_schluessel'), array($institut_id, $img_id, $schluessel));
						} else {
			...
			// Datenbankeintrag
			if ( mysqli_num_rows($img_sql) == 1 ) {
				$image_upload 	= sql_update('images', array('img_titel', 'img_url', 'img_beschreibung', 'chg_user'), array($img_titel, $img_url, $img_beschreibung, $user_email), array('img_institut', 'img_id', 'img_schluessel'), array($institut_id, $img_id, $schluessel));
			} else {
				$image_upload 	= sql_insert('images', array('img_institut', 'img_schluessel', 'img_titel', 'img_url', 'img_beschreibung', 'new_user', 'chg_user'), array($institut_id, $schluessel, $img_titel,  $img_url, $img_beschreibung, $user_email, $user_email));
			}
			if ($image_upload == true) {
				$img_beschreibung 	= str_replace('<br />', '', $img_beschreibung);
				$img_beschreibung 	= $_POST['img_beschreibung'];
...
<ol class="breadcrumb">
	...
	<li class="breadcrumb-item active">Auszeichnung bearbeiten</li>
</ol>


<div class="jumbotron">
	<h1>Auszeichnung bearbeiten</h1>
	<p class="lead">Hier können Sie eine weitere Auszeichnung bearbeiten.</p>
</div>
		...
		<div class="col-md-12">
			<div class="card mb-3">
				<div class="card-header">
					<i class="fa fa-picture-o" aria-hidden="true"></i> &nbsp; Angaben
				</div>
				<div class="card-body">
					<div class="row">
						<div class="col-md-6">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control" placeholder="URL" name="img_url" value="<?php echo $img_url; ?>">
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group">
