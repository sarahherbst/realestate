<?php
	...
	$berater_sql 			= sql_select_where('all', 'berater', 'ber_id', $ber_id, '', '');
	$berater_row 			= mysqli_fetch_assoc($berater_sql);
	$ber_gruppe 			= $berater_row['ber_gruppe'];
	$ber_anrede 			= $berater_row['ber_anrede'];
	$ber_vorname 			= $berater_row['ber_vorname'];
	$ber_nachname 			= $berater_row['ber_nachname'];
	$ber_position 			= $berater_row['ber_position'];
	$ber_filiale 			= $berater_row['ber_filiale'];
	$ber_strasse 			= $berater_row['ber_strasse'];
	$ber_plz 				= $berater_row['ber_plz'];
	$ber_ort 				= $berater_row['ber_ort'];
	$ber_tel 				= $berater_row['ber_tel'];
	$ber_fax 				= $berater_row['ber_fax'];
	$ber_email 				= $berater_row['ber_email'];

	$begr_sql 				= sql_select_where('all', 'beratergruppe', array('begr_institut', 'begr_id'), array($institut_id, $ber_gruppe), '', '');
	$begr_row 				= mysqli_fetch_assoc($begr_sql);
	$begr_name 				= $begr_row['begr_name'];

	$img_sql 				= sql_select_where('all', 'images', array('img_institut', 'img_schluessel', 'img_item_id'), array($institut_id, $schluessel, $ber_id), '', '');
	...
	if (isset($_POST['submit'])) {
		...
		$berater_update = sql_update('berater', array('ber_gruppe', 'ber_anrede', 'ber_vorname', 'ber_nachname', 'ber_position', 'ber_filiale', 'ber_strasse', 'ber_plz', 'ber_ort', 'ber_tel', 'ber_fax', 'ber_email', 'chg_user'), array($ber_gruppe, $ber_anrede, $ber_vorname, $ber_nachname, $ber_position, $ber_filiale, $ber_strasse, $ber_plz, $ber_ort, $ber_tel, $ber_fax, $ber_email, $user_email), 'ber_id', $ber_id);
		...
		if ($_FILES['motiv']['error'] <= 0) {
				...
				if ($fehler == 0) {
					...
					if ($thumbnail == '' || $thumbnail == false) {
						...
						if ( mysqli_num_rows($img_sql) == 1 ) {
							$image_upload 	= sql_update('images', array('img_bild', 'img_thumb', 'img_beschreibung', 'chg_user'), array($large, $thumbnail, $img_beschreibung, $user_email), array('img_institut', 'img_schluessel', 'img_item_id'), array($institut_id, $schluessel, $ber_id));
						} else {
			...
			// Datenbankeintrag
			if ( mysqli_num_rows($img_sql) == 1 ) {
				$image_upload 		= sql_update('images', array('img_beschreibung', 'chg_user'), array($img_beschreibung, $user_email), array('img_institut', 'img_schluessel', 'img_item_id'), array($institut_id, $schluessel, $ber_id));
			} else {
				$image_upload 		= sql_insert('images', array('img_institut', 'img_schluessel', 'img_item_id', 'img_beschreibung', 'new_user', 'chg_user'), array($institut_id, $schluessel, $ber_id, $img_beschreibung, $user_email, $user_email));
			}
			if ($image_upload == true) {
				$img_beschreibung 	= str_replace('<br />', '', $img_beschreibung);
				$img_beschreibung 	= $_POST['img_beschreibung'];
...
<ol class="breadcrumb">
	...
	<li class="breadcrumb-item active">Berater bearbeiten</li>
</ol>

<div class="jumbotron">
	<h1>Berater bearbeiten</h1>
	<p class="lead">Hier können Sie den ausgewählten Ansprechpartner bearbeiten.</p>
</div><!-- /.jumbotron -->
							...
							<div class="form-row">
								<div class="form-group col-md-12">
									...
									<select class="custom-select" name="ber_gruppe" required>
										<option value=0 disabled>Bitte wählen Sie ein Team</option>
										<?php $begr_sql = sql_select_where('all', 'beratergruppe', 'begr_institut', $institut_id, '', ''); ?>
										<?php while ($begr_row = mysqli_fetch_assoc($begr_sql)) { ?>
											<option value="<?php echo $begr_row['begr_id']; ?>" <?php echo ($begr_name == $begr_row['begr_name'] ? 'selected' : ''); ?>><?php echo $begr_row['begr_name']; ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
