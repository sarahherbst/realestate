<?php
	...
	if (isset($_POST['submit'])) {
		...
		if ($fehler == 0) {
			$speichern 		= sql_insert('texte', array('txt_institut', 'txt_rubrik', 'txt_schluessel', 'txt_titel', 'txt_einleitung', 'txt_auszug', 'txt_beitrag', 'new_user', 'chg_user'), array($institut_id, $rub_id, $schluessel, $txt_titel, $txt_einleitung, $txt_auszug, $txt_beitrag, $user_email, $user_email));
		}

		if ($speichern == true) {
			$alert .= '<div class="alert alert-success" role="alert">Das Testimonial für die Rubrik <b>'.$row_rubrik['rub_name'].'</b> wurde erfolgreich hinzugefügt.</div>';
			$txt_titel 			= '';
			$txt_einleitung 	= '';
			$txt_auszug 		= '';
			$txt_beitrag 		= '';
			...
			$fehler++;
			$alert   .= '<div class="alert alert-danger alert-dismissible fade in" role="alert"><b>Fehler!</b> Das Testimonial für die Rubrik <b>'.$row_rubrik['rub_name'].'</b> konnte nicht hinzugefügt werden.</div>';
			...
			if ($fehler == 0) {
				...
				if ($fehler == 0) {
					...
					if ($thumbnail == '' || $thumbnail == false) {
						...
						// Datenbankeintrag
						$article_sql 	= sql_select_where('txt_id', 'texte', array('txt_institut', 'txt_rubrik', 'txt_schluessel', 'txt_titel'), array($institut_id, $rub_id, $schluessel, $_POST['txt_titel']), '', '' );
...
<ol class="breadcrumb">
	...
	<li class="breadcrumb-item active">Testimonial hinzufügen</li>
</ol>

<div class="jumbotron">
	<h1>Testimonial hinzufügen</h1>
	<p class="lead">Hier haben Sie die Möglichkeit ein Testimonial für die Rubrik <b><?php echo $row_rubrik['rub_name']; ?></b> hinzuzufügen.</p>
</div><!-- /.jumbotron -->
